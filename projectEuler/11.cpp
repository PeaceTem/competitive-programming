...
int main(){
    ...
    for(int j=0;j<=20-4;j++){
        for(int k=0;k<20;k++){
            // product = grid[j][k]*grid[j+1][k]*grid[j+2][k]*grid[j+3][k] > product ? grid[j][k]*grid[j+1][k]*grid[j+2][k]*grid[j+3][k] :  product;
            if(grid[j][k]*grid[j+1][k]*grid[j+2][k]*grid[j+3][k] > product){
                product = grid[j][k]*grid[j+1][k]*grid[j+2][k]*grid[j+3][k];
                cout << grid[j][k] << " " << grid[j+1][k] << " " << grid[j+2][k] << " " << grid[j+3][k] << " => " << product << '\n';
                cout << j << "," << k << " " << j+1 << "," << k << " " << j+2 << "," << k << " " << j+3 << ',' << k << " => " << product << '\n';
            }
        }
    }
    ...
    for(int j=0;j<20;j++){
        for(int k=0;k<20-3;k++){
            product = grid[j][k]*grid[j][k+1]*grid[j][k+2]*grid[j][k+3] > product ? grid[j][k]*grid[j][k+1]*grid[j][k+2]*grid[j][k+3] :  product;
        }
    }
    // forward diagonal
    for(int j=0;j<20-3;j++){
        for(int k=0;k<20-3;k++){
            product = grid[j][k]*grid[j+1][k+1]*grid[j+2][k+2]*grid[j+3][k+3] > product ? grid[j][k]*grid[j+1][k+1]*grid[j+2][k+2]*grid[j+3][k+3] :  product;
        }
    }
